...
| `dplyr`   	| `if_else`    	| A vectorized conditional function.         	| 
| `dplyr`   	| `case_when`  	| A general vectorized conditional function. 	| 
...
: Table 1. Functions in the `dsTidyverse()` package {#tbl:functions} 

Note that dsTidyverse supports non-standard evaluation[REF]. The name of the server-side data frame is passed in quotes to `df.name`, and the variable names ("sex" and "bmi") are evaluated as columns within "my_data". Various helper functions can also be used within the `tidy_expr` argument, for example `all_of` and `any_of` to specify multiple variables in filter conditions. See examples at the end of this section for example use of dsTidyverse, and the package vignette for a more detailed guide.
