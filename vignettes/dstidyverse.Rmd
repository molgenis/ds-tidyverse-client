...
dslite.server$assignMethod("case_whenDS", "case_whenDS")
dslite.server$assignMethod("bind_rowsDS", "bind_rowsDS")
dslite.server$assignMethod("bind_colsDS", "bind_colsDS")
...
#> ✖ [1m[33msim1[39m: <text>:1:81: unexpected '$' 1: dplyr::case_when(mtcars$mpg < 20 $TILDE$ "low", mtcars$mpg >= 20 & mtcars$mpg < $ ^[22m
```
...
#> Error:
#> ! There are some DataSHIELD errors:
#> ✖ [1m[33msim1[39m: ✖ `case_whenDS` returned the following error: ℹ $ operator is invalid for atomic vectors[22m
```

``` r

ds.table("recoded_missing")
