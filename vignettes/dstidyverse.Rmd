...
## Set up DSLite environement and log in.
...
```
### Rename
`ds.rename` renames columns within a serverside dataframe. Column names are passed unquoted and are
...
### Mutate
`ds.mutate` creates new columns in a serverside dataframe. These new columns are normally transformations
of existing columns. This reduces the number of steps currently required in DataSHIELD, i.e. creating
...
#### Choose where the new columns are positioned in the dataframe with `.before` and `.after`:
